// Standard five-field cron parsing, plain-English describing and next-run search

export type CronPart = {
  // Bare * (a step may still apply)
  any: boolean;
  from: number;
  to: number;
  step: number;
...
export type CronField = {
  parts: CronPart[];
  // Expanded allowed values, sorted (day-of-week normalized to 0-6)
  values: number[];
  // Plain * with no step, which drives the dom/dow or-rule and the phrasing
  any: boolean;
};
...
export type CronResult = { ok: true; cron: Cron; tokens: string[] } | { ok: false; error: string };
...
export const FIELD_DEFS: FieldDef[] = [
  ...
  { key: "dow", label: "day of week", min: 0, max: 7, names: "sun mon tue wed thu fri sat".split(" ") },
];
...
export function parseCron(input: string): CronResult {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    for (const src of tokens[i].split(",")) {
      ...
      if (step < 1) return { ok: false, error: `${def.label}: step must be at least 1` };
      ...
      if (m[1] === "*") {
        ...
        if (lo == null || hi == null) return { ok: false, error: `${def.label}: can't read "${src}"` };
        if (lo > hi) return { ok: false, error: `${def.label}: range ${lo}-${hi} is backwards` };
        from = lo;
        to = hi;
      } else {
        const v = resolve(m[1], def);
        if (v == null) return { ok: false, error: `${def.label}: can't read "${src}"` };
        from = v;
      ...
      if (from < def.min || to > def.max)
        return { ok: false, error: `${def.label}: "${src}" is out of range (${def.min}-${def.max})` };

      parts.push({ any, from, to, step });
      for (let v = from; v <= to; v += step) values.add(def.key === "dow" ? v % 7 : v);
    }
...
const partPhrase = (p: CronPart, cfg: PhraseCfg): string => {
  ...
  if (p.any) return p.step > 1 ? `every ${ord(p.step)} ${cfg.unit}` : `every ${cfg.unit}`;
  if (p.from === p.to) return cfg.name ? name(p.from) : `${cfg.unit} ${p.from}`;
  if (p.step > 1) return `every ${ord(p.step)} ${cfg.unit} from ${name(p.from)} through ${name(p.to)}`;
  return cfg.name ? `${name(p.from)} through ${name(p.to)}` : `${cfg.unit}s ${p.from} through ${p.to}`;
};

export function fieldPhrase(f: CronField, cfg: PhraseCfg): string {
  const singles = f.parts.every((p) => !p.any && p.from === p.to);
  if (singles) {
    const names = f.parts.map((p) => (cfg.name ? cfg.name(p.from) : `${p.from}`));
    const label = cfg.name ? "" : f.parts.length > 1 ? `${cfg.unit}s ` : `${cfg.unit} `;
...
export function describeCron(c: Cron): string {
  const singles = (f: CronField) => f.parts.every((p) => !p.any && p.from === p.to);

  let s: string;
  if (singles(c.minute) && singles(c.hour) && c.minute.values.length * c.hour.values.length <= 4) {
    // Clean clock times while the combination count stays readable
    ...
    // "at minute 5" alone reads ambiguous, "every 5th minute" does not
  ...
  if (!c.dow.any) s += `${c.dom.any ? "" : " and"} on ${fieldPhrase(c.dow, PHRASE_CFGS.dow)}`;
  return s;
}
...
export function nextRuns(cron: Cron, count: number, from: Date): Date[] {
  ...
  // Vixie cron quirk: when both dom and dow are restricted, either matching counts
  ...
  // Walk days, then allowed hours and minutes inside each, bounded at 5 years
...
export const cronPresets: CronPreset[] = [
  ...
  { slug: "every-5-minutes", label: "Every 5 minutes", expression: "*/5 * * * *" },
  { slug: "every-10-minutes", label: "Every 10 minutes", expression: "*/10 * * * *" },
  { slug: "every-15-minutes", label: "Every 15 minutes", expression: "*/15 * * * *" },
  { slug: "every-30-minutes", label: "Every 30 minutes", expression: "*/30 * * * *" },
  ...
  { slug: "every-12-hours", label: "Every 12 hours", expression: "0 */12 * * *" },
  { slug: "every-day-at-midnight", label: "Every day at midnight", expression: "0 0 * * *" },
  { slug: "every-day-at-9am", label: "Every day at 9am", expression: "0 9 * * *" },
  { slug: "every-day-at-noon", label: "Every day at noon", expression: "0 12 * * *" },
  ...
  { slug: "first-of-the-month", label: "First of the month", expression: "0 0 1 * *" },
  { slug: "every-quarter", label: "Every quarter", expression: "0 0 1 */3 *" },
  { slug: "every-year", label: "Every year", expression: "0 0 1 1 *" },
];

export const getCronPreset = (slug: string) => cronPresets.find((p) => p.slug === slug);

const normalize = (expr: string) => expr.trim().toLowerCase().split(/\s+/).join(" ");
