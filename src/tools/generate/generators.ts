// Random generators, one entry = one indexable page at /generate/[slug]
...
const CARD_BRANDS = {
  ...
  mastercard: { prefixes: ["51", "52", "53", "54", "55"], length: 16, cvcLength: 3 },
...
export const generators: Generator[] = [
  {
    ...
    keywords: "password generator, strong password, secure random password, create password online",
    about:
    ...
    generate: (opts) => {
      const pool = (["lowercase", "uppercase", "digits", "symbols"] as const).filter((k) => opts[k]);
      ...
      return Array.from({ length: opts.count as number }, () => {
        ...
        const chars = [
          ...
          ...Array.from({ length: Math.max(0, length - pool.length) }, () => pick(all)),
        ];
  ...
  },
  {
    ...
    mono: true,
    options: [{ key: "count", label: "How many", type: "number", min: 1, max: 50, defaultValue: 10 }],
    generate: (opts) => Array.from({ length: opts.count as number }, () => `#${randomHexBytes(3)}`),
  },
  {
    slug: "hex-string",
    label: "Hex String",
    ...
    keywords: "random hex string, random bytes, hex token generator, secret key generator",
    about:
    ...
    generate: (opts) =>
      Array.from({ length: opts.count as number }, () => randomHexBytes(opts.bytes as number)),
  },
  {
    slug: "lorem-ipsum",
    label: "Lorem Ipsum",
    ...
    options: [
      ...
      { key: "sentences", label: "Sentences each", type: "number", min: 1, max: 15, defaultValue: 5 },
    ],
    generate: (opts) =>
      Array.from({ length: opts.paragraphs as number }, (_, i) => {
        const sentences = Array.from({ length: opts.sentences as number }, loremSentence);
  ...
  },
  {
    ...
    keywords: "random name generator, fake name generator, placeholder names, test names, sample names",
    about:
    ...
    options: [{ key: "count", label: "How many", type: "number", min: 1, max: 50, defaultValue: 10 }],
    generate: (opts) =>
      Array.from({ length: opts.count as number }, () => `${pickFrom(FIRST_NAMES)} ${pickFrom(LAST_NAMES)}`),
  },
  {
    slug: "email",
    label: "Email",
    ...
    keywords: "random email generator, fake email, test email address, placeholder email, sample emails",
    about:
    ...
    generate: (opts) =>
      Array.from({ length: opts.count as number }, () => {
        ...
        const local = pickFrom([`${f}.${l}`, `${f}${l}${randomInt(100)}`, `${f[0]}${l}`, `${f}_${l}`]);
  ...
  },
  {
    ...
    about:
      "All numbers come from the 555-01XX block, which the North American Numbering Plan reserves for fictional use — the same range films and TV use. Nothing generated here can ring a real phone.",
    mono: true,
    options: [
      { key: "count", label: "How many", type: "number", min: 1, max: 50, defaultValue: 10 },
      { key: "countryCode", label: "+1 prefix", type: "boolean", defaultValue: false },
    ],
    generate: (opts) =>
      Array.from({ length: opts.count as number }, () => {
        // Any area code except n11 service codes, always the fictional 555-0100..0199 block
        let area: number;
        do {
          area = 200 + randomInt(800);
        } while (area % 100 === 11);
        const line = `01${`${randomInt(100)}`.padStart(2, "0")}`;
        return opts.countryCode ? `+1 ${area} 555 ${line}` : `(${area}) 555-${line}`;
      }),
  },
  {
    slug: "ip-address",
    ...
    keywords: "random ip address generator, fake ip, test ip address, random ipv4, random ipv6, private ip",
    about:
      "IPv4 addresses avoid loopback and, unless the toggle says otherwise, the private RFC 1918 ranges. IPv6 addresses come from 2001:db8::/32, the prefix RFC 3849 reserves for documentation, so generated addresses route nowhere by design.",
    mono: true,
    options: [
      { key: "count", label: "How many", type: "number", min: 1, max: 50, defaultValue: 10 },
      { key: "ipv6", label: "IPv6", type: "boolean", defaultValue: false },
      { key: "privateRange", label: "Private range", type: "boolean", defaultValue: false },
    ],
    generate: (opts) =>
      Array.from({ length: opts.count as number }, () => {
        if (opts.ipv6) {
          // 2001:db8::/32 is reserved for documentation, so these route nowhere
          const groups = Array.from({ length: 6 }, () => randomInt(0x10000).toString(16));
        ...
        do {
          a = 1 + randomInt(223);
          b = r();
        } while (a === 10 || a === 127 || (a === 172 && b >= 16 && b <= 31) || (a === 192 && b === 168));
        return `${a}.${b}.${r()}.${r()}`;
      }),
  },
  {
    ...
    keywords: "random mac address generator, fake mac address, test mac address, locally administered mac",
    about:
    ...
    generate: (opts) =>
      Array.from({ length: opts.count as number }, () => {
        ...
        const mac = bytes.map((b) => b.toString(16).padStart(2, "0")).join(opts.dashes ? "-" : ":");
  ...
  },
  {
    ...
    description:
      "Free online test credit card number generator. Luhn-valid numbers for Visa, Mastercard, Amex and Discover — they pass checkout form validation but are not issued cards, so they cannot be charged.",
    keywords:
    ...
    options: [
      ...
      { key: "mastercard", label: "Mastercard", type: "boolean", defaultValue: true },
      { key: "amex", label: "Amex", type: "boolean", defaultValue: true },
      { key: "discover", label: "Discover", type: "boolean", defaultValue: true },
      { key: "details", label: "Expiry + CVC", type: "boolean", defaultValue: true },
    ],
    generate: (opts) => {
      const brands = (Object.keys(CARD_BRANDS) as (keyof typeof CARD_BRANDS)[]).filter((b) => opts[b]);
  ...
  },
  {
    ...
    keywords: "random date generator, random date, test dates, sample dates, iso date generator",
    about:
    ...
    options: [
      { key: "count", label: "How many", type: "number", min: 1, max: 50, defaultValue: 10 },
      { key: "fromYear", label: "From", type: "number", min: 1900, max: 2100, defaultValue: 1970 },
      { key: "toYear", label: "To", type: "number", min: 1900, max: 2100, defaultValue: 2030 },
    ],
    generate: (opts) => {
