// JWT decoding only: no verification and no network, so the token never leaves the page
...
export function decodeJwt(token: string): DecodedJwt {
  const parts = token
    ...
    .split(".");
  if (parts.length !== 3)
    return { ok: false, error: `expected 3 dot-separated sections, got ${parts.length}` };
  ...
  if (!payload) return { ok: false, error: "payload isn't valid base64url json" };
...
// A harmless sample so the page demos itself; the signature is fake
