...
// One entry per transform; each one is its own page at /text/[slug]
...
// Split into words for case conversions: handles camel, snake, kebab and spaces
...
const byLine = (fn: (lines: string[]) => string[]) => (input: string) => fn(input.split(/\r?\n/)).join("\n");
...
export const textTransforms: TextTransform[] = [
  ...
  },
  {
    ...
    group: "Change Case",
    apply: (x) => x.replace(/\S+/g, (w) => w.charAt(0).toUpperCase() + w.slice(1)),
  },
  {
    slug: "sentence-case",
    label: "Sentence case",
    ...
    group: "Change Case",
    apply: (x) => x.toLowerCase().replace(/(^\s*|[.!?]\s+)([a-z])/g, (_, p, c) => p + c.toUpperCase()),
  },
  {
    slug: "camel-case",
    label: "camelCase",
  ...
  },
  {
    ...
    description: "Free online slug generator. Convert any text into a clean, lowercase, URL-safe slug.",
...

  {
    ...
    description: "Free online tool to trim leading and trailing whitespace from every line of your text.",
  ...
  },
  {
    ...
    description: "Free online tool to collapse repeated spaces and tabs into single spaces.",
  ...
  },
  {
    ...
    description: "Free online tool to remove all line breaks and join text into a single continuous line.",
  ...
  },
  {
    ...
    description: "Free online tool to strip blank and whitespace-only lines from your text.",
  ...
  },
  {
    ...
    description: "Free online tool to deduplicate lines in a list while preserving the original order.",
  ...
  },
  {
    ...
    description: "Free online tool to sort the lines of your text alphabetically.",
  ...
  },
  {
    ...
    label: "Reverse Text",
    title: "Reverse Text",
    description: "Free online tool to reverse the characters of your text. Works with unicode and emoji.",
  ...
  },
  {
    ...
    description: "Free online Base64 decoder. Convert Base64 back to plain text, right in your browser.",
    keywords: "base64 decode, base64 to text, decode base64 online",
    group: "Data Formats",
    placeholder: "aGVsbG8gd29ybGQ=",
    apply: (x) => new TextDecoder().decode(Uint8Array.from(atob(x.trim()), (c) => c.charCodeAt(0))),
  },
  {
    slug: "url-encode",
    label: "URL Encode",
    title: "URL Encode Text",
    description: "Free online URL encoder. Percent-encode text for safe use in URLs and query strings.",
  ...
  },
  {
    ...
    description: "Free online URL decoder. Decode percent-encoded URLs and query strings back to plain text.",
...
export const getTextTransform = (slug: string) => textTransforms.find((t) => t.slug === slug);

// First-appearance order for the switcher groups
export const textTransformGroups = Array.from(new Set(textTransforms.map((t) => t.group)));
