// Icon set presets: the sizes to render plus the config files that reference them
...
export const iconPresets: IconPreset[] = [
  {
    slug: "pwa",
    label: "Web / PWA",
    description: "Favicons, apple touch icon and PWA manifest icons with a ready-to-use site.webmanifest.",
    files: [
  ...
  },
  {
    ...
    description: "Launcher icon mipmaps for every density bucket, ready to drop into res/.",
    files: [
...
export const getIconPreset = (slug: string) => iconPresets.find((p) => p.slug === slug);
