// Hash digests, one algorithm per page. SHA comes from WebCrypto; MD5 is
// hand-rolled because WebCrypto refuses it and it is still widely asked for.
...
export type HashAlgo = {
  ...
  label: string;
  bits: number;
  // Shown under the result, mostly to warn about the broken ones
  note?: string;
  about: string;
...
export const hashAlgos: HashAlgo[] = [
  ...
  },
  {
    ...
    bits: 256,
    about:
      'SHA-256 is the workhorse of the SHA-2 family — a 256-bit digest with no practical attacks. It is the default choice for integrity checks, signatures, content addressing and most things labeled "checksum" today.',
  ...
  },
  {
    ...
    bits: 384,
    about:
      "SHA-384 is SHA-512 truncated to 384 bits with different initial values, which also makes it immune to length-extension attacks. It is common in TLS certificate chains and anywhere a middle ground between SHA-256 and SHA-512 is wanted.",
  ...
  },
  {
    ...
    bits: 512,
    about:
      "SHA-512 produces a 512-bit digest and is often faster than SHA-256 on 64-bit hardware because it processes bigger words. Use it for the widest SHA-2 security margin.",
    digest: (input) => subtleDigest("SHA-512", input),
  },
];

export const getHashAlgo = (slug: string) => hashAlgos.find((a) => a.slug === slug);
