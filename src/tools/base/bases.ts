// Number base conversions, one from/to pair per page. BigInt underneath, so a
// 256-bit hash converts as happily as 255.

export type NumberBase = {
  slug: string;
  label: string;
  radix: 2 | 8 | 10 | 16;
  // Prefix people paste in (0x and friends), stripped on parse
  prefix: string;
  sample: string;
  digits: RegExp;
  about: string;
...
export const getBase = (slug: string) => numberBases.find((b) => b.slug === slug);
...
export const getBasePair = (slug: string) => basePairs.find((p) => p.slug === slug);
