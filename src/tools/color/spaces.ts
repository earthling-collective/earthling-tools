// Hand-rolled color space conversions. Every space parses to and formats from
// sRGB floats with alpha, so any pair round-trips through one representation.
...
// Björn Ottosson's Oklab matrices, wrapped to polar chroma and hue
...
// Out-of-gamut values clamp per channel, matching what browsers render
...
// Pull numeric tokens out of whatever punctuation was typed
const nums = (input: string) =>
  (input.match(/-?\d*\.?\d+%?/g) ?? []).map((t) => ({ value: parseFloat(t), pct: t.endsWith("%") }));

const trim = (x: number, decimals: number) => Number(x.toFixed(decimals));

// A fourth token, when present, is alpha: raw 0–1 or a percentage
...
export const colorSpaces: ColorSpace[] = [
  {
    ...
    parse: (input) => {
      ...
      if (!m) return null;
      let hex = m[1];
      // 3 and 4 digit shorthand doubles up; 4 and 8 digit forms carry alpha
  ...
  },
  {
    ...
    parse: (input) => {
      ...
      const ch = t.slice(0, 3).map(({ value, pct }) => clamp01(pct ? value / 100 : value / 255));
  ...
  },
  {
    ...
    parse: (input) => {
      const t = nums(input);
      if (t.length !== 3 && t.length !== 4) return null;
      return [...hslToRgb(t[0].value, clamp01(t[1].value / 100), clamp01(t[2].value / 100)), alphaFrom(t)];
    },
    format: ([r, g, b, a]) => {
  ...
  },
  {
    ...
    parse: (input) => {
      ...
      return [...oklchToRgb(L, Math.max(0, t[1].value), t[2].value), alphaFrom(t)];
    },
    format: ([r, g, b, a]) => {
...
export const getSpace = (slug: string) => colorSpaces.find((s) => s.slug === slug);

// An explicit prefix identifies a pasted color whatever the selected mode is
...
// Compact ?color= form: bare hex digits or comma-joined numbers, nothing that
// needs percent-encoding. The pair in the path pins the space.
...
export const getColorPair = (slug: string) => colorPairs.find((p) => p.slug === slug);
